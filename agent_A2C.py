...
def prepareEnv(ifc_filename = "", objectScaling = 1.0):
  ...
  num_cpu = 36  # Number of processes to use
  ...
  return DummyVecEnv([make_env(env_id, i, ifcpath, scaling=objectScaling) for i in range(num_cpu)])
...
if __name__ == "__main__":
  if do_train:
    ...
    #model = A2C.load("a2c" env=env, tensorboard_log="./log/")
    ...
    env = prepareEnv()
    model = PPO2.load("a2c", env=env, tensorboard_log="./log/")
