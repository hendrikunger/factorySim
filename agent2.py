...
def prepareEnv(ifc_filename = "", objectScaling = 1.0):
  ...
  num_cpu = 24  #52  # Number of processes to use
...
if __name__ == "__main__":
  if do_train:
    ...
    model.learn(total_timesteps=2000, tb_log_name="Batch_1",reset_num_timesteps=True, callback=TensorboardCallback())
    ...
    for i in range(1,6):
      ...
      model.set_env(env)
      model.learn(total_timesteps=2000, tb_log_name=f"Batch_{i+1}",reset_num_timesteps=True, callback=TensorboardCallback())
