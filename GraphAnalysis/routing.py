...
class FactoryPath():
    ...
    def calculateAll(self, multi, bb):
        if self.TIMING:
        ...
        distances = np.arange(0,  bb.boundary.length, self.boundarySpacing)
        points = [ bb.boundary.interpolate(distance) for distance in distances]

        #Points on Machines
        distances = np.arange(0,  multi.boundary.length, self.boundarySpacing)
        ...
        if total:
            ...
            if endpoints_to_prune:
                nodes_to_prune = self.pruneAlongPath(self.fullPathGraph, starts=endpoints_to_prune, ends=self.crossroads, min_length=2 * self.minDeadEndLength)
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(ITERATIONS)):
        multi, bb = FactoryCreator(*baseConfigs.SMALLSQUARE.creationParameters()).create_factory()
        machine_colors = [rng.random(size=3) for _ in multi.geoms]
        factoryPath = FactoryPath()
        factoryPath.TIMING = True
        factoryPath.PLOTTING = True
        factoryPath.calculateAll(multi, bb)
